  ...
  default_costmaps_{"global_costmap"}
{
  ...
  for (size_t i = 0; i < planner_costmaps_.size(); i++) {
    ...
    costmap_ros = std::make_shared<nav2_costmap_2d::Costmap2DROS>(
      planner_costmaps_[i], std::string{get_namespace()}, planner_costmaps_[i]);
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      ...
      RCLCPP_INFO(
        get_logger(), "Created global planner plugin %s of type %s with costmap %s",
        planner_ids_[i].c_str(), planner_types_[i].c_str(), costmap_name.c_str());
...
PlannerServer::on_activate(const rclcpp_lifecycle::State & state)
{
  ...
  for (PlannerMap::iterator it = planners_.begin(); it != planners_.end(); ++it) {
    it->second->activate();
  }
