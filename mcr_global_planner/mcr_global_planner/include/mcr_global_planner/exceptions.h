...
{
  return "(" + std::to_string(pose.pose.position.x) + ", " + std::to_string(pose.pose.position.y) + ", " + 
...
{
public:
  explicit GlobalPlannerException(const std::string& description) : nav2_core::PlannerException(description) {}
};
...
public:
  ...
  InvalidStartPoseException(const geometry_msgs::msg::PoseStamped& pose, const std::string& problem) :
...
public:
  ...
  InvalidGoalPoseException(const geometry_msgs::msg::PoseStamped& pose, const std::string& problem) :
