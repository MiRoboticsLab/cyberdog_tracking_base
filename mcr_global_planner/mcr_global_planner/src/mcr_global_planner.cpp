    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".potential_calculator", rclcpp::ParameterValue("mcr_planner_plugins::AStar"));
  node_->get_parameter(name_ + ".potential_calculator", plugin_name);
  RCLCPP_INFO(node_->get_logger(), "MCRGlobalPlanner Using PotentialCalculator \"%s\"", plugin_name.c_str());
  ...
  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".traceback", rclcpp::ParameterValue("mcr_planner_plugins::GradientPath"));
  ...
  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".path_caching", rclcpp::ParameterValue(false));
  node_->get_parameter(name_ + ".path_caching", path_caching_);

  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".improvement_threshold", rclcpp::ParameterValue(-1.0));
  node_->get_parameter(name_ + ".improvement_threshold", improvement_threshold_);
  cached_path_cost_ = -1.0;

  bool publish_potential;

  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".publish_potential", rclcpp::ParameterValue(false));
  ...
  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".print_statistics", rclcpp::ParameterValue(false));
...
geometry_msgs::msg::Pose2D MCRGlobalPlanner::transformPose(const geometry_msgs::msg::PoseStamped & in_pose,
...
nav_msgs::msg::Path MCRGlobalPlanner::createPlan(
  ...
  nav_2d_msgs::msg::Path2D path = traceback_->getPath(potential_grid_, 
  ...
  {
    RCLCPP_INFO(node_->get_logger(), "DluxGlobalPlanner Got plan! Cost: %.2f, %d updated "
...
bool MCRGlobalPlanner::hasValidCachedPath(const geometry_msgs::msg::Pose2D& /*local_goal*/,
...
bool MCRGlobalPlanner::shouldReturnNewPath(const nav_2d_msgs::msg::Path2D& /*new_path*/, const double new_path_cost) const{
  return new_path_cost + improvement_threshold_ <= cached_path_cost_;
}

using rcl_interfaces::msg::ParameterType;
void MCRGlobalPlanner::on_parameter_event_callback(const rcl_interfaces::msg::ParameterEvent::SharedPtr event){
