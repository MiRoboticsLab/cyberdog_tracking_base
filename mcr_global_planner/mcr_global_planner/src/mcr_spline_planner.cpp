#include <mcr_global_planner/mcr_spline_planner.h>
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node_, name_ + ".spline_name", rclcpp::ParameterValue(
      "bezier"));
  node_->get_parameter(name_ + ".spline_name", spliner_name_);

  std::string plugin;
  nav2_util::declare_parameter_if_not_declared(
    node_, name_ + "." + spliner_name_ + ".plugin", rclcpp::ParameterValue(
      "mcr_planner_plugins::BezierSpliner"));
  node_->get_parameter(name_ + "." + spliner_name_ + ".plugin", plugin);

  RCLCPP_INFO(
    node_->get_logger(), "MCRSplinePlanner Using Spline Type \"%s\"",
    spliner_name_.c_str());
...
{
  RCLCPP_INFO(node_->get_logger(), "Cleaning up plugin %s of type MCRSplinePlanner", name_.c_str());
}

void MCRSplinePlanner::activate()
{
  RCLCPP_INFO(node_->get_logger(), "Activating plugin %s of type MCRSplinePlanner", name_.c_str());
}

void MCRSplinePlanner::deactivate()
{
  RCLCPP_INFO(
    node_->get_logger(), "Deactivating plugin %s of type MCRSplinePlanner",
    name_.c_str());
}
...
PLUGINLIB_EXPORT_CLASS(mcr_global_planner::MCRSplinePlanner, nav2_core::GlobalPlanner)
