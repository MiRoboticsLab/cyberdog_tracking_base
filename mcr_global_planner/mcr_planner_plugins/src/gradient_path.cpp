  ...
  mcr_global_planner::CostInterpreter::Ptr cost_interpreter)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name_ + ".iteration_factor", rclcpp::ParameterValue(
      4.0));
...
nav_2d_msgs::msg::Path2D GradientPath::getPath(
  ...
  double & path_cost)
{
  ...
  mcr_nav_grid::Index index(static_cast<unsigned int>(x_int_part),
  ...
  mcr_nav_grid::Index finish_index(static_cast<unsigned int>(finish_x),
    static_cast<unsigned int>(finish_y));

  unsigned int c = 0,
    max_iterations = static_cast<unsigned int>(potential_grid.size() * iteration_factor_);
...
bool GradientPath::shouldGridStep(
  const mcr_global_planner::PotentialGrid & potential_grid,
...
mcr_nav_grid::Index GradientPath::gridStep(
  const mcr_global_planner::PotentialGrid & potential_grid,
  const mcr_nav_grid::Index & index)
{
...
void GradientPath::calculateGradient(
  const mcr_global_planner::PotentialGrid & potential_grid,
  const mcr_nav_grid::Index & index)
{
  ...
  float north_p = index.y <
    potential_grid.getHeight() - 1 ? potential_grid(index.x, index.y + 1) : HIGH_POTENTIAL;
  float west_p = index.x > 0 ? potential_grid(index.x - 1, index.y) : HIGH_POTENTIAL;
  float east_p = index.x <
    potential_grid.getWidth() - 1 ? potential_grid(index.x + 1, index.y) : HIGH_POTENTIAL;
