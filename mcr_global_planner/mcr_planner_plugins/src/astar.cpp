...
{
void AStar::initialize(rclcpp_lifecycle::LifecycleNode::SharedPtr node, 
                       ...
                       mcr_global_planner::CostInterpreter::Ptr cost_interpreter)
{
  ...
  name_ = name;
  nav2_util::declare_parameter_if_not_declared(node ,name_ + ".manhattan_heuristic", rclcpp::ParameterValue(false));
  node->get_parameter(name_ + ".manhattan_heuristic", manhattan_heuristic_);
  
  nav2_util::declare_parameter_if_not_declared(node, name + ".use_kernel", rclcpp::ParameterValue(true));
  node->get_parameter(name + ".use_kernel", use_kernel_);
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".minimum_requeue_change", rclcpp::ParameterValue(1.0));
  node->get_parameter(name_ + ".minimum_requeue_change", minimum_requeue_change_);
  
}

unsigned int AStar::updatePotentials(mcr_global_planner::PotentialGrid& potential_grid,
...
void AStar::add(mcr_global_planner::PotentialGrid& potential_grid, double prev_potential,
...
float AStar::getHeuristicValue(const mcr_nav_grid::Index& index, const mcr_nav_grid::Index& start_index) const
{
