...
#include "mcr_tracking_components/recoveries/spin_and_search.hpp"
...
void SpinAndSearch::onConfigure()
{
...
nav2_recoveries::Status SpinAndSearch::onRun(const std::shared_ptr<const SpinAction::Goal> command)
{
...
nav2_recoveries::Status SpinAndSearch::onCycleUpdate()
{
  geometry_msgs::msg::PoseStamped current_pose;
  if (!nav2_util::getCurrentPose(
      current_pose, *tf_, global_frame_, robot_base_frame_,
...
bool SpinAndSearch::isCollisionFree(
...
PLUGINLIB_EXPORT_CLASS(mcr_tracking_components::SpinAndSearch, nav2_core::Recovery)
